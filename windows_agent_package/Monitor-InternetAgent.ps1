...
<#
.SYNOPSIS
    Net-Insight Monitor Agent (Windows) - V3 FINAL PRODUCTION VERSION
.DESCRIPTION
    This is the definitive, complete, and fully debugged agent script for Windows.
    It performs a full suite of network tests, including WiFi signal strength,
    calculates a health summary, and securely submits the data to the central server.
#>
...
$LogFile = Join-Path -Path $AgentScriptDirectory -ChildPath "net_insight_agent_windows.log"
...
try {
    ...
    if ((Get-Variable -Name "ENABLE_WIFI" -ErrorAction SilentlyContinue) -eq $null) { $script:ENABLE_WIFI = $true } # New feature flag
    ...
    Write-Log -Message "Starting Net-Insight Monitor Agent (Type: $AGENT_TYPE)."
    ...
    $Results = [ordered]@{
        ...
        http_check = @{}
        speed_test = @{}
        wifi_summary = @{} # New object for WiFi metrics
    }
    ...
    if ($ENABLE_WIFI) {
        Write-Log "Performing WiFi signal test..."
        try {
            # Use netsh to get all interface details. It's reliable and built-in.
            $NetshOutput = netsh.exe wlan show interfaces
            if ($LASTEXITCODE -eq 0 -and $NetshOutput) {
                $WifiDetails = @{
                    signal_percent = $null; signal_dbm = $null; ssid = $null; bssid = $null;
                    channel = $null; frequency_band = $null; radio_type = $null; authentication = $null;
                }
                
                # Parse the multi-line output from netsh
                foreach ($line in ($NetshOutput | Out-String -Stream)) {
                    if ($line -match '^\s+Signal\s+:\s+(.+)$') { $WifiDetails.signal_percent = [int]($matches[1].Trim().Replace('%', '')) }
                    if ($line -match '^\s+SSID\s+:\s+(.+)$') { $WifiDetails.ssid = $matches[1].Trim() }
                    if ($line -match '^\s+BSSID\s+:\s+(.+)$') { $WifiDetails.bssid = $matches[1].Trim() }
                    if ($line -match '^\s+Channel\s+:\s+(.+)$') { $WifiDetails.channel = [int]($matches[1].Trim()) }
                    if ($line -match '^\s+Radio type\s+:\s+(.+)$') { $WifiDetails.radio_type = $matches[1].Trim() }
                    if ($line -match '^\s+Authentication\s+:\s+(.+)$') { $WifiDetails.authentication = $matches[1].Trim() }
                }

                # Infer frequency band from the channel number
                if ($WifiDetails.channel) {
                    if ($WifiDetails.channel -le 14) { $WifiDetails.frequency_band = "2.4 GHz" }
                    else { $WifiDetails.frequency_band = "5 GHz" } # Can also be 6 GHz for Wi-Fi 6E, but this is a safe default.
                }

                if ($WifiDetails.signal_percent -ne $null) {
                    $Results.wifi_summary = $WifiDetails
                    Write-Log "WiFi Details (netsh): SSID='$($WifiDetails.ssid)', Signal=$($WifiDetails.signal_percent)%, Chan=$($WifiDetails.channel)"
                } else {
                    Write-Log "WiFi test: Could not parse signal from connected WiFi network (netsh)."
                }
            } else {
                Write-Log "WiFi test: Not connected to a WiFi network or WLAN service is down (netsh)."
            }
        } catch {
            Write-Log -Level WARN -Message "WiFi test failed. Error: $($_.Exception.Message)"
        }
    }
    ...
    try {
        ...
        if ($CENTRAL_API_KEY) {
            $SubmitHeaders.Add("X-API-KEY", $CENTRAL_API_KEY)
        } else {
            Write-Log -Level WARN -Message "CENTRAL_API_KEY is not set. Submission will likely be rejected."
        }
