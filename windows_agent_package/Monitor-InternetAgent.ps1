...
try {
    ...
    if ($ENABLE_WIFI) {
        ...
        try {
            ...
            $SignalLine = $NetshOutput | Select-String -Pattern "^\s+Signal"
            if ($SignalLine) {
                $SignalPercent = ($SignalLine -split ':')[1].Trim().Replace('%', '')
                $Results.wifi_summary = @{
                    signal_percent = [int]$SignalPercent
                    signal_dbm = $null # netsh does not provide dBm
                }
                Write-Log "WiFi Signal (netsh): $SignalPercent %"
            } else {
                Write-Log "WiFi test: Not connected to a WiFi network (netsh)."
            }
        } catch {
