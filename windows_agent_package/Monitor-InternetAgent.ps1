...
try {
...
}) { $WifiDetails.signal_percent = [int]($matches[1].Trim().Replace('%', '')) }
    ...
    if (Test-Path $LockFile) { Remove-Item $LockFile -Force -ErrorAction SilentlyContinue }
}) { $WifiDetails.ssid = $matches[1].Trim() }
    ...
    if (Test-Path $LockFile) { Remove-Item $LockFile -Force -ErrorAction SilentlyContinue }
}) { $WifiDetails.bssid = $matches[1].Trim() }
    ...
    if (Test-Path $LockFile) { Remove-Item $LockFile -Force -ErrorAction SilentlyContinue }
}) { $WifiDetails.channel = [int]($matches[1].Trim()) }
    ...
    if (Test-Path $LockFile) { Remove-Item $LockFile -Force -ErrorAction SilentlyContinue }
}) { $WifiDetails.radio_type = $matches[1].Trim() }
    ...
    if (Test-Path $LockFile) { Remove-Item $LockFile -Force -ErrorAction SilentlyContinue }
}) { $WifiDetails.authentication = $matches[1].Trim() }
                }
